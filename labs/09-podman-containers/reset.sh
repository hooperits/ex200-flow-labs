...
# Reset system state for Module 09
...
if [ -d "/home/vagrant/.config/systemd/user" ]; then
    echo "Deteniendo y deshabilitando el servicio de systemd del usuario..."
...
if command -v podman &>/dev/null; then
    if podman ps -a --filter name=web-server --format "{{.Names}}" | grep -q "web-server"; then
        echo "Deteniendo y eliminando contenedor web-server..."
