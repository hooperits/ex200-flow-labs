...
Write-Host "         Instalador Automatico de rsync para Windows (Winget)" -ForegroundColor Cyan
Write-Host "======================================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($rsyncCmd) {
    Write-Host "[OK] rsync ya esta configurado en tu sistema en: $($rsyncCmd.Source)" -ForegroundColor Green
    Exit 0
}
...
if (-not $wingetCmd) {
    Write-Host "[Error] winget (Windows Package Manager) no esta disponible en tu sistema." -ForegroundColor Red
...
Write-Host "Instalando cwRsync Client a traves de winget..." -ForegroundColor Yellow
...
if ($installedPath) {
    Write-Host ""
    Write-Host "[OK] cwRsync Client se instalo correctamente en: $installedPath" -ForegroundColor Green
    ...
    if ($currentPath -notlike "*$installedPath*") {
        ...
        Write-Host "[OK] PATH actualizado de forma permanente." -ForegroundColor Green
    }
    ...
    Write-Host ""
    Write-Host "[Error] La instalacion de winget termino, pero no se encontro rsync.exe en los directorios estandar." -ForegroundColor Red
